<script lang="ts">
	...
	import { ListService, type ShoppingListItem } from '$lib/list.service';
	import {
		buffer,
		debounceTime,
		filter,
	...
	const tileClick$ = new Subject<{ listId: string; itemId: string }>();
	subscriptions.add(
		tileClick$
			.pipe(
				...
				mergeMap(([item, uid]) => ListService.deleteItem(uid, item.listId, item.itemId))
			)
			.subscribe()
	);
	...
	const handleListAdd = (event: Event, listId: string) => {
		const form = event.target;
		if (!(form instanceof HTMLFormElement)) throw TypeError();
		const formData = new FormData(form);
		const todo = formData.get('item')!.toString().trim();
		Firebase.uid$
			.pipe(
				first(),
...
<ul>
	{#each $lists$ as list}
		<li class="card">
			...
			<div id="contents">
				<form
					on:submit|preventDefault={(event) => handleListAdd(event, list.key)}
					autocomplete="off"
				>
				...
				<ul>
					{#each list.data as item}
						<li
							...
							class="item"
							on:click={() => tileClick$.next({ itemId: item.key, listId: list.key })}
							on:keydown={() => {}}
						>
							<span>{item.item}</span>
